		...
		<section id="projects" className="px-8 pt-20 container mx-auto py-300">
			...
			<div className="flex justify-center mt-20 flex-wrap gap-10">
				{dataProjects.map((item, index) => (
					<div
						key={index}
						className=" w-90  flex flex-col justify-between gap-3 bg-panel text-white rounded-lg overflow-hidden p-3 gap-5 outline-1 shadow-[0px_0px_5px_#ffffff] hover:shadow-[0px_0px_8px_#ffffff]  hover:-translate-y-2 hover:bg-panel-hover duration-1000 group"
					>
						<div className="h-50 overflow-hidden rounded-lg ">
							<Image src={item.image} alt={item.title} width={500} height={500} className="h-full w-120 w-30" />
						</div>
						...
						<div className="flex flex-col gap-2 grow">
							...
							<div className="flex items-center gap-3 flex-wrap pb-3">
								{item.tech.slice(0, 4).map((tech, index) => (
									<div key={index} className="text-xs font-semibold  p-2 text-nowrap text-center bg-secondary rounded-sm first:bg-gray-700/75">
										{tech}
									</div>
								))}
						...
						<button
							...
							className="bg-primary hover:bg-secondary p-1 font-semibold rounded-md duration-300 outline-1"
						>
							See Project
						</button>
					</div>
